shinyServer(function(input, output, session) {
  ...
  # Calculating number of participants to achieve enough power for the certain effect
  pwr_80 <- reactive({
  ...
  output$power <- renderPlot({
    max_n <- min(max(60, pwr::pwr.t.test(d = d_pwr(), sig.level = .05,power = .9)$n),
                 200)

    pwrs <- data.frame(ns = seq(5, max_n, 5), # from 5 to max_n with steps of 5
  ...
  output$conditional_text <- renderText({"There are less than two studies, which have investigated this feature. Please choose a different feature."
  })
  output$conditional_text_2 <- renderText({"If you would like to contribute with a study, see CONTRIBUTIONLINK"
  })
  # Do the below instead (to get the URL)
  ...
  output$conditional_results <- renderUI({
    if(nrow(pwrdata()) > 1){
      list(
        fluidRow(
          valueBoxOutput("power_d", width = 4),
      ...
      box(width = NULL, fluidRow(column(width = 10, textOutput("conditional_text"), br(), textOutput("conditional_text_2"))))
    }
  })
})
