...
#### Overview
MetaLab is a research tool for aggregating across studies in the cognitive development literature. Currently, MetaLab contains `r as.integer(sum(as.numeric(dataset_info$num_experiments), na.rm = TRUE))` effect sizes across
meta-analyses in `r nrow(dataset_info)` domains of cognitive development, based on
data from `r as.integer(sum(as.numeric(dataset_info$num_papers), na.rm = TRUE))` papers collecting
`r as.integer(sum(as.numeric(dataset_info$num_subjects), na.rm = TRUE))` subjects. These studies
