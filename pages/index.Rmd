    ...
    box(
      ...
      a(href = link, class = "no-format",
        fluidRow(class = "domain-box",
          ...
          column(width = 12, class = "dataset-txt", align = "center",
...
fluidRow(class = "domain-info",
  column(width = 6,
    map(1, #seq(1, ceiling(length(domains) / 2), 1),
      ~ tagList(
        fluidRow(
  ...
  column(
    width = 6,
    map(2, #seq(ceiling(length(domains) / 2 + 1), length(domains), 1),
        ~ tagList(
          fluidRow(
...
fluidRow(class = "domain-info",
  column(
    width = 12,
    map(3, #seq(ceiling(length(domains) / 2 + 1), length(domains), 1),
        ~ tagList(
          fluidRow(
            domainBox(., domains, c(3, 3), link = paste0("domain/", domains[[.]][["id"]], ".html"))),
          fluidRow(class = "bottom-part",
            column(width = 3),
            column(width = 6, valueBoxes(
              filter(ungroup(dataset_info), domain == domain_ids[[.]]) %>% metricsCounter)
              ),
            column(width = 3)
            )
